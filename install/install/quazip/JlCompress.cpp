...
//        qDebug()<<QString::number(mb);
//        if (mb>50){return false;}
    }
    return true;
}
static bool xrenderCopyData(QIODevice &inFile, QIODevice &outFile,const bool& isCancel)
{
    while (!inFile.atEnd()) {
        char buf[4096];
        qint64 readLen = inFile.read(buf, 4096);
...
bool JlCompress::xrenderCompressOneFile(QuaZip* zip, QString fileName, QString fileDest,const bool& isCancel){
    if (!zip) return false;
    if (zip->getMode()!=QuaZip::mdCreate &&
        zip->getMode()!=QuaZip::mdAppend &&
        zip->getMode()!=QuaZip::mdAdd) return false;
    ...
    // Apro il file risulato
    QuaZipFile outFile(zip);
    if(!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileDest, inFile.fileName()),0,0,8,1)) return false;

    // Copio i dati
    if (!xrenderCopyData(inFile,outFile,isCancel) || outFile.getZipError()!=UNZ_OK) {
        return false;
    }
...
bool JlCompress::compressFiles(QString fileCompressed, QStringList files) {
    ...
    Q_FOREACH (QString file, files) {
        info.setFile(file);
        bool e = info.exists();
        if (!e || !compressFile(&zip,file,info.fileName())) {
            QFile::remove(fileCompressed);
            return false;
        }
    }
...
bool JlCompress::xrenderCompress(QString fileCompressed, QString file,const bool& isCancel){
    // Creo lo zip
    QuaZip zip(fileCompressed);
    QDir().mkpath(QFileInfo(fileCompressed).absolutePath());
    if(!zip.open(QuaZip::mdCreate)) {
    ...
    info.setFile(file);
    bool e = info.exists();
    if (!e || !xrenderCompressOneFile(&zip,file,info.fileName(),isCancel)) {
        QFile::remove(fileCompressed);
        return false;
    }
...
QStringList JlCompress::XGTextractDir(QString fileCompressed, QString dir, QObject* obj)
{
    // Apro lo zip
    QuaZip zip(fileCompressed);
    if(!zip.open(QuaZip::mdUnzip)) {
